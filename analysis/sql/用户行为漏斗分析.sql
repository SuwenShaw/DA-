...
from (
         (
             select count(url) as sum_pv,null as sum_fav,null as sum_cart,null as sum_buy
             from (
                      select url
                      from web_action_log
                      union all
         ...
         union all
         (select null as sum_pv,
                 sum(case when action_type = 'fav' then 1 else 0 end) as sum_fav,
                 null as sum_cart,null as sum_buy
          from behavior_log
          )
         union all
         (select null as sum_pv,null as sum_fav,
                 sum(case when action_type = 'cart' then 1 else 0 end) as sum_cart,
                 null as sum_buy
          from behavior_log
          )
         union all
         (select null as sum_pv,null as sum_fav,null as sum_cart,
