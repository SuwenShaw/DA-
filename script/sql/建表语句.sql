...
(
    `id`           int          NOT NULL,
    `user_id`      varchar(255) NULL,
    `source`       varchar(20)  NULL COMMENT '来源: 如 web app',
    `action_time`  timestamp    NULL COMMENT '动作时间',
    `action_type`  varchar(255) NULL COMMENT '动作类型:如加购物车，收藏',
    `sku_code`     varchar(255) NULL COMMENT '产品编码',
    `os`           varchar(255) NULL COMMENT '操作系统',
    `device_id`    varchar(255) NULL COMMENT '设备号，如果是web则为cookie',
    `device_type`  varchar(255) NULL COMMENT '设备类型: imei idfa cookie',
    `ip`           varchar(255) NULL COMMENT 'ip地址',
    `province`     varchar(255) NULL COMMENT 'ip 地址解析出来的省份',
    `city`         varchar(255) NULL COMMENT 'ip 地址解析出来的城市',
    `created_time` timestamp    NULL COMMENT '创建时间',
...
ALTER TABLE `app_action_log`
    ADD CONSTRAINT `fk_app_action_log_fact_person_info_1` FOREIGN KEY (`user_id`) REFERENCES `fact_person_info` (`person_id`);
ALTER TABLE `dim_order_info`
    ADD CONSTRAINT `fk_dim_order_info_fact_order_detail_info_1` FOREIGN KEY (`order_no`) REFERENCES `fact_order_detail_info` (`order_no`);
ALTER TABLE `fact_delivery_address_info`
    ADD CONSTRAINT `fk_fact_delivery_address_info_dim_order_info_1` FOREIGN KEY (`delivery_address_id`) REFERENCES `dim_order_info` (`delivery_address_id`);
ALTER TABLE `fact_order_detail_info`
    ADD CONSTRAINT `fk_fact_order_detail_info_fact_store_info_1` FOREIGN KEY (`store`) REFERENCES `fact_store_info` (`store_no`);
ALTER TABLE `fact_order_detail_info`
    ADD CONSTRAINT `fk_fact_order_detail_info_fact_store_info_copy_1_1` FOREIGN KEY (`sku_code`) REFERENCES `fact_product_info` (`sku_code`);
ALTER TABLE `fact_person_info`
    ADD CONSTRAINT `fk_fact_person_info_dim_order_info_1` FOREIGN KEY (`person_id`) REFERENCES `dim_order_info` (`person_id`);
ALTER TABLE `moible_device_mapping`
    ADD CONSTRAINT `fk_moible_device_mapping_ad_app_log_1` FOREIGN KEY (`device_id`) REFERENCES `ad_app_log` (`device_id`);
ALTER TABLE `moible_device_mapping`
    ADD CONSTRAINT `fk_moible_device_mapping_fact_person_info_1` FOREIGN KEY (`mobile_md5`) REFERENCES `fact_person_info` (`mobile`);
ALTER TABLE `web_action_log`
    ADD CONSTRAINT `fk_web_action_log_fact_person_info_1` FOREIGN KEY (`user_id`) REFERENCES `fact_person_info` (`person_id`);
ALTER TABLE `behavior_log`
    ADD CONSTRAINT `fk_behavior_log_fact_person_info_1` FOREIGN KEY (`user_id`) REFERENCES `fact_person_info` (`person_id`);
